...
interface EventProductSectionProps {
  ...
  getProductTypesForColor?: (colorName: string) => ProductType[];
...
const EventProductSection = ({ 
  title, 
  colors, 
  productTypes: staticProductTypes, 
  getProductTypesForColor,
  sizeGuideType = 'both', 
  showColorNote = false 
  ...
  const productTypes = getProductTypesForColor 
    ? getProductTypesForColor(selectedColor.name) 
    : (staticProductTypes || []);
  ...
  // Update product type and size when color changes (for dynamic product types)
  useEffect(() => {
    if (getProductTypesForColor) {
      const newProductTypes = getProductTypesForColor(selectedColor.name);
      setSelectedProductType(newProductTypes[0]);
      setSelectedSize(newProductTypes[0]?.sizes[0]);
    }
  }, [selectedColor, getProductTypesForColor]);
  ...
  if (!selectedProductType || !selectedSize) {
    return null;
  }
  ...
  return (
    <div className="bg-muted/30 border border-border rounded-lg p-5 h-full transition-all duration-300 hover:shadow-lg hover:border-primary/20 hover:bg-muted/40">
      <h3 className="font-display text-lg font-medium text-foreground mb-1 text-center sm:text-left">{title}</h3>
      <p className="text-xs text-foreground/60 font-body mb-4 text-center sm:text-left">3 Day Hire</p>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-3">
            ...
            {selectedProductType.sizes.length === 1 ? (
              <p className="text-sm font-body text-foreground">{selectedSize.label} - £{selectedSize.price.toFixed(2)} <span className="text-foreground/60 text-xs">inc. VAT</span></p>
            ) : (
                ...
                <SelectContent className="min-w-[280px]">
                  {selectedProductType.sizes.map((size) => (
                    <SelectItem key={size.value} value={size.value}>
                      {size.label} - £{size.price.toFixed(2)} inc. VAT
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
        ...
        <div className="flex-1 flex flex-col justify-between">
          ...
          <div className="mb-3">
            ...
            <p className="text-xs text-foreground/60 font-body mt-1">
              Total: <span className="font-medium text-foreground">£{totalPrice.toFixed(2)}</span> <span className="text-foreground/60">inc. VAT</span>
            </p>
          </div>
